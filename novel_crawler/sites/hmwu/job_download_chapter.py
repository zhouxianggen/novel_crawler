...
class Job(BaseJob):
    ...
    def run(self):
        ...
        with self.session_scope() as session:
            stmt = ("SELECT uuid, url FROM hmwu_chapter "
    ...
    def process(self, session, uuid, url):
        ...
        ps = []
        for img in doc.xpath('.//div[@class="comicpage"]/div/img[@class="lazy"]'):
            href = img.attrib['data-original']
            self.log.info("href {}".format(href))
            ps.append({"img": href})
        ps = json.dumps(ps, ensure_ascii=False)
        self.log.info('update chapter paragraphs')
        stmt = ("UPDATE hmwu_chapter SET paragraphs=:paragraphs, "
